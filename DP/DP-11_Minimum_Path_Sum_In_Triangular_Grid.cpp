...
int f(int row, int col, int n, vector<vector<int>>& triangle, vector<vector<int>> dp) {
    ...
    if(row > n || col > triangle[row].size()-1) return 0;
    if(dp[row][col] != -1) return dp[row][col];
    int dr;
    dr = triangle[row][col] + f(row+1, col, n, triangle, dp);
    int rr;
    rr = triangle[row][col] + f(row+1, col+1, n, triangle, dp);
    return dp[row][col] = min(dr, rr);
}
