...
int f(int indx, int buy, vector<int> &prices, vector<vector<int>> &dp) {
    ...
    int profit = INT_MIN;
    if(buy) {
        profit = max(profit, max(-prices[indx] + f(indx+1, 0, prices, dp),
                                0 + f(indx+1, buy, prices, dp)));
    }
    else {
        profit = max(profit, max(prices[indx] + f(indx+2, 1, prices, dp),
                            0 + f(indx+1, 0, prices, dp)));
    }
