 ...
 * 1 2 3 4 5 
 * */
int f(int i, int W, vector<int> &val, vector<int> &weights, vector<vector<int>> &dp) {
    if(i == 0) {
        if(weights[i] <= W) return (W / weights[i]) * val[0];
        return 0;
    }
    if(dp[i][W] != -1) return dp[i][W];
    int pick = 0;
    if(weights[i] <= W) pick = val[i] + f(i, W-weights[i], val, weights, dp);

    int notPick = f(i-1, W, val, weights, dp);

    return dp[i][W] = max(pick, notPick);
}
int knapsack(vector<int> &val, vector<int>& weights, int maxW) {
    int n = val.size();
    vector<vector<int>> dp(n, vector<int>(maxW+1, -1));
    return f(n-1, maxW, val, weights, dp);
}
void solve() {
    int n, t;
    cin >> n >> t;
