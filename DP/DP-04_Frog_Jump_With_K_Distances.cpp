...
int f(int ind, const vector<int>& height, vector<int>& dp, int k) {
    if (ind == 0) return 0;
    ...
    for(int i = 1; i <= k; i++) {
        if (ind-i >= 0) {
            int jump = f(ind - i, height, dp, k) + abs(height[ind] - height[ind - i]);
...
int frogJump(const vector<int>& height, int k) {
