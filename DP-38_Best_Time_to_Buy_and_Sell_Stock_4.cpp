...
int f(int indx, int buy, int cap, vector<int> &prices, vector<vector<vector<int>>> &dp) {
    ...
    int profit = INT_MIN;
    if(buy) {
        profit = max(profit, max(-prices[indx] + f(indx+1, 0, cap, prices, dp),
                                0 + f(indx+1, buy, cap, prices, dp)));
    }
    else {
        profit = max(profit, max(prices[indx] + f(indx+1, 1, cap-1, prices, dp),
                            0 + f(indx+1, 0, cap, prices, dp)));
    }
