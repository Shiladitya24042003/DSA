 ...
 * 1 2 3 4 5 
 * */
int f(int i, int len, vector<int> &nums, vector<vector<int>> &dp) {
    if(i == 0) {
        if(i+1 <= len) return (len / (i+1))*nums[i];
        return 0;
    }
    if(dp[i][len] != -1) return dp[i][len];

    int pick = 0;
    if(i+1 <= len) pick = nums[i] + f(i, len-i-1, nums, dp);
    int notPick = 0 + f(i-1, len, nums, dp);

    return dp[i][len] = max(pick, notPick);
}
int rodCutting(vector<int> &nums) {
    int n = nums.size();
